...
/* Write a secret artifact (the Ed25519 signing key) with no race window: O_EXCL
 * refuses to follow a pre-existing symlink or reuse an attacker-planted file, and
 * the 0600 mode is applied at create time so the key hex is never world-readable
 * (unlike write_file's fopen-then-chmod).  Fails if the file already exists. */
static int write_secret_file(const char *path, const void *data, size_t len) {
   int fd = open(path, O_WRONLY | O_CREAT | O_EXCL, 0600);
   if (fd < 0) {
      fprintf(stderr, "ota-keytool: cannot create %s (remove it first if it exists): %s\n", path,
              strerror(errno));
      return FAILURE;
   }
   FILE *f = fdopen(fd, "wb");
...
static int cmd_keygen(int argc, char **argv) {
   ...
   if (write_secret_file(path, hex, strlen(hex)) != SUCCESS) {
...
static int cmd_pubkey_header(int argc, char **argv) {
   ...
   /* Emit the OTA_PUBKEY_HEX shape the ESP32 sketch consumes (ota_apply.cpp /
    * ota_pubkey.h.example) — a hex-string array, NOT a uint8_t[][32] blob (no
    * consumer reads that form; the Tier-1 RPi reads hex lines from
    * /etc/dawn/ota_pubkey instead). */
   fprintf(f, "/* Auto-generated by ota-keytool — do not edit.\n");
   fprintf(f, " * OTA verify keyring baked into the Tier-2 ESP32 device image. */\n");
   fprintf(f, "#ifndef OTA_PUBKEY_H\n#define OTA_PUBKEY_H\n\n");
   fprintf(f, "#define OTA_PUBKEY_COUNT %d\n\n", n);
   fprintf(f, "static const char *const OTA_PUBKEY_HEX[OTA_PUBKEY_COUNT] = {\n");
   for (int k = 0; k < n; k++) {
      char hex[OTA_PK_BYTES * 2 + 1];
      sodium_bin2hex(hex, sizeof(hex), keys[k], OTA_PK_BYTES);
      fprintf(f, "   \"%s\", /* key %d%s */\n", hex, k, k == 0 ? " (current)" : "");
   }
   fprintf(f, "};\n\n#endif /* OTA_PUBKEY_H */\n");
   fclose(f);
